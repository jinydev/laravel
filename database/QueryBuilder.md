...
이 메소드는 한번에 결과에 대한 하나의 작은 `chunk` 를 획득하고, 각각의 `chunk`를 `Closure` 를 통해서 처리합니다. 
...
다음의 괄호 안에 제약조건을 그룹으로 묶는 예제를 살펴보겠습니다:
...
또한 한번의 `insert` 메소드 호출로 테이블에 여러 레코드를 추가하기 위해서 배열의 배열을 전달 할 수도 있습니다. 
